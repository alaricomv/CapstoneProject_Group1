...
export class ProductService {
  ...
  getProductByTag(productId: string): Observable<Product> {
    return this.http.get<Product>(PRODUCT_BY_TAG_URL + productId);
  }
  getProductByProductName(productName: string): Observable<Product> {
    return this.http.get<Product>(PRODUCT_BY_NAME_URL + productName);
  }
  ...
  register(productRegister: IProductRegister): Observable<Product> {
    return this.http.post<Product>(PRODUCT_REGISTER_URL, productRegister).pipe(
      tap({
        next: (user) => {
          this.toastrService.success('Product Register Succesful');
        },
        error: (errorResponse) => {
          this.toastrService.error(
            errorResponse.error,
            'Product register Failed'
          );
        },
      })
    );
  ...
  editProduct(productRegister: IProductModify): Observable<Product> {
    return this.http.put<Product>(PRODUCT_MODIFY_URL, productRegister).pipe(
      tap({
        next: (user) => {
          this.toastrService.success('Product Modification Succesful');
        },
        error: (errorResponse) => {
  ...
  deleteProductById(productId: string): Observable<Product> {
    return this.http.delete<Product>(PRODUCT_DELETE_URL + productId).pipe(
      tap({
        next: (user) => {
          this.toastrService.success('Product Deletion Succesful');
        },
        error: (errorResponse) => {
          this.toastrService.error(
            errorResponse.error,
